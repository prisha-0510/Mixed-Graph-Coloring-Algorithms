...
def predict_colors(model, adj_list, time_budget=500, max_queue_size=4):
    """
    ...
    class GraphState:
        def __init__(self, adj_list, labels=None, colors=0):
            ...
            self._avg_closeness_centrality = None  # Cache for average closeness centrality
        ...
        def get_colors(self):
            """Return number of colors (cached by default since it's a property)"""
            return self.colors
            
        def get_average_closeness_centrality(self):
            """Return average closeness centrality of unlabeled subgraph"""
            if self._avg_closeness_centrality is not None:
            ...
            # Handle edge cases
            if num_nodes <= 1:
                self._avg_closeness_centrality = 0
                return 0
            ...
            for i in range(num_nodes):
                # BFS to find shortest paths from node i to all other nodes
                ...
                while queue:
                    current = queue.pop(0)
                    for neighbor in subgraph_adj_list[current]:
                        if distances[neighbor] == -1:
                            distances[neighbor] = distances[current] + 1
                            queue.append(neighbor)
                
                # Calculate closeness centrality for node i
                # Closeness = 1 / (sum of distances to all other reachable nodes)
                reachable_distances = [d for d in distances if d > 0]
                if reachable_distances:
                    closeness = 1.0 / sum(reachable_distances)
                else:
                    closeness = 0.0
            ...
            if centralities:
                self._avg_closeness_centrality = sum(centralities) / len(centralities)
            else:
                self._avg_closeness_centrality = 0
    ...
    def update_queue(Q, new_state, max_size):
        """
        Updates queue to maintain only max_size states with highest average closeness centrality
        """
        # Get all states including the new one
        states = list(Q.queue) + [new_state]
        
        # Sort states by average closeness centrality (higher is better, so reverse=True)
        states.sort(key=lambda x: x.get_average_closeness_centrality(), reverse=True)
        
        # Keep only the first max_size states (highest average closeness centrality)
    ...
    while not Q.empty():
        # Select state with maximum average closeness centrality from queue
        states = list(Q.queue)
        ...
        # Modified selection strategy with randomness for tie-breaking
        # First, compute the maximum average closeness centrality
        max_avg_centrality = max(state.get_average_closeness_centrality() for state in states)
        
        # Find all states that have this maximum average closeness centrality
        max_centrality_states = [state for state in states if state.get_average_closeness_centrality() == max_avg_centrality]
        
        # Randomly select one of these states
        current_state = random.choice(max_centrality_states)
